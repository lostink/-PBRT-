...
//        shape = std::make_shared<Mesh>();
    ...
    else if(QString::compare(lgtType, QString("PointLight")) == 0)
    {
        Color3f lightColor = ToVec3(geometry["lightColor"].toArray());
        Float intensity = static_cast< float >(geometry["intensity"].toDouble());
        lightType = std::make_shared<PointLight>(shape->transform, lightColor * intensity, shape);
    }
    else if(QString::compare(lgtType, QString("SpotLight")) == 0)
    {
        Color3f lightColor = ToVec3(geometry["lightColor"].toArray());
        Float intensity = static_cast< float >(geometry["intensity"].toDouble());
        Float TotalWidth = (30.f),FalloffStart = (15.f);
        if (geometry.contains(QString("TotalWidth"))) TotalWidth = geometry["TotalWidth"].toDouble();
        if (geometry.contains(QString("FalloffStart"))) FalloffStart = geometry["FalloffStart"].toDouble();

        lightType = std::make_shared<SpotLight>(shape->transform, lightColor * intensity, shape, TotalWidth, FalloffStart);
    }
    else
    {
        std::cout << "Could not parse the geometry!" << std::endl;
...
Camera JSONReader::LoadCamera(QJsonObject& camera)
{
    ...
    float g;
    if(camera.contains(QString("sigma_a"))) sigma_a = ToVec3(camera["sigma_a"].toArray());
    if(camera.contains(QString("sigma_s"))) sigma_s = ToVec3(camera["sigma_s"].toArray());
    if(camera.contains(QString("g"))) g = camera["g"].toDouble();

    Camera result;
    result.medium = new HomogeneousMedium(sigma_a,sigma_s,g);
    ...
    if(camera.contains(QString("RealisticCamera"))) result.RealisticCamera = camera["RealisticCamera"].toBool();
