...
{
    Intersection itsect;
    itsect.point = Point3f(transform.T() * glm::vec4(WarpFunctions::squareToDiskUniform(xi), 1));
    itsect.normalGeometric = glm::normalize(transform.invTransT() * Normal3f(0,0,1));
    *pdf = 1.0f / Area();
    return itsect;
}
...
{
    Bounds3f ret(Point3f(-1.f, -1.f, 0.f), Point3f(1.f, 1.f, 0.f));
